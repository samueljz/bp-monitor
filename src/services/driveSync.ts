...
const FILE_NAME = 'BPMonitorBackup.json'          // separate file from GlucoMonitorBackup.json
...
export function syncData(promptUser: boolean = false) {
  ...
  syncTimeout = window.setTimeout(async () => {
    ...
    try {
      ...
      // Gather all bpm_ prefixed local storage data, excluding token credentials
      ...
      if (fileId) {
        ...
        if (remoteData) {
          ...
          if (promptUser) {
            prioritizeRemote = window.confirm(
              'Do you want to overwrite your local data with the backup from Google Drive?\n\nClick OK to overwrite local data.\nClick Cancel to keep local data and merge.'
            )
          } else {
