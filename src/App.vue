...
import {
  NConfigProvider, NCard, NButton, NModal,
  NTabs, NTabPane, NDropdown,
} from 'naive-ui'
...
import {
  syncData, isSignedIn, isSyncing, syncError, needsReauth,
  handleAuthClick, handleSignoutClick, initGoogleApi,
...
import { formatTime }  from './utils/bp'

// ─── Theme ────────────────────────────────────────────────────────────────────

const { isDarkMode, naiveTheme, themeOverrides, initTheme, toggleTheme } = useTheme()

// ─── Clock ────────────────────────────────────────────────────────────────────

const { timeParts, dateString, todayDateKey } = useClock()

// ─── Readings ─────────────────────────────────────────────────────────────────

const { todayReadings, loadToday, addReading, updateReading } = useReadings()

// Reload today's readings whenever the date changes (midnight rollover)
watch(todayDateKey, loadToday)

// ─── Log modal ────────────────────────────────────────────────────────────────

const logModal = useLogModal((entry, editingIndex) => {
  if (editingIndex >= 0) updateReading(editingIndex, entry)
  else                   addReading(entry)
...
const latestReading  = computed(() => todayReadings.value[0] ?? null)
...
// ─── Drive conflict-resolution modal ─────────────────────────────────────────

const showConflictModal = computed({
  get:  () => !!pendingConflict.value,
  set:  (val) => { if (!val) resolveConflict(false) },
})
...
onMounted(() => {
  ...
  // Refresh today's readings after a Drive sync
  window.addEventListener('bpm_sync_complete', loadToday)

  // Init Google API once both gapi and google.accounts are loaded
...
<template>
  <n-config-provider :theme="naiveTheme" :theme-overrides="themeOverrides">
    ...
    >
        ...
        <n-tabs type="segment" justify-content="space-evenly" class="mb-2 flex flex-col h-full" pane-wrapper-style="flex: 1; min-height: 0;" pane-style="height: 100%;">
          ...
          <n-tab-pane name="today" tab="Today" display-directive="show">
            <div class="w-full h-full overflow-y-auto no-scrollbar px-4 pb-32 pt-2 flex flex-col gap-4 transition-colors duration-500">
              ...
              <!-- Empty state -->
              <template v-if="todayReadings.length === 0">
                <div class="flex flex-col items-center justify-center gap-3 py-16 px-4">
                  <div class="w-16 h-16 rounded-full flex items-center justify-center" :class="isDarkMode ? 'bg-slate-800' : 'bg-slate-100'">
              ...
              <template v-else>
                <div @click="logModal.openEdit(latestReading!, 0)" class="cursor-pointer">
                  <BloodPressureCard
      ...
      <!-- ── Log Modal ────────────────────────────────────────────────────── -->
      <n-modal v-model:show="logModal.showModal.value">
        <n-card
          style="width: 340px; border-radius: 28px;"
          :title="logModal.editingIndex.value >= 0 ? 'Edit Reading' : 'Log Blood Pressure'"
          :bordered="false"
          size="huge"
          role="dialog"
          aria-modal="true"
        ...
        >
          <div class="flex flex-col gap-6 pt-2">
            ...
            <div>
              ...
              <div class="flex gap-2">
                <input
                  type="time"
                  v-model="logModal.formTimeString.value"
                ...
                />
                <button type="button" @click="logModal.setTimeToNow()" class="shrink-0 px-4 rounded-2xl bg-indigo-500/10 hover:bg-indigo-500/20 active:scale-95 text-indigo-600 dark:text-indigo-400 font-extrabold text-xs uppercase tracking-widest transition-all">
                  Now
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="flex items-center gap-3 bg-slate-100 dark:bg-slate-800 rounded-2xl px-3 py-2.5">
                <button type="button" @click="logModal.adjustValue('systolic', -1)" class="shrink-0 w-12 h-12 rounded-xl bg-white dark:bg-slate-700 shadow-sm active:scale-95 transition-transform flex items-center justify-center text-2xl font-black text-slate-600 dark:text-slate-200">−</button>
                <div class="flex-1 flex flex-col items-center min-w-0">
                  <input type="number" inputmode="numeric" step="1" min="0" max="300" v-model.number="logModal.formSystolic.value" placeholder="120" class="w-full bg-transparent text-center text-4xl font-black tabular-nums text-slate-800 dark:text-white outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
            ...
            <div>
              ...
              <div class="flex items-center gap-3 bg-slate-100 dark:bg-slate-800 rounded-2xl px-3 py-2.5">
                <button type="button" @click="logModal.adjustValue('diastolic', -1)" class="shrink-0 w-12 h-12 rounded-xl bg-white dark:bg-slate-700 shadow-sm active:scale-95 transition-transform flex items-center justify-center text-2xl font-black text-slate-600 dark:text-slate-200">−</button>
                <div class="flex-1 flex flex-col items-center min-w-0">
                  <input type="number" inputmode="numeric" step="1" min="0" max="200" v-model.number="logModal.formDiastolic.value" placeholder="80" class="w-full bg-transparent text-center text-4xl font-black tabular-nums text-slate-800 dark:text-white outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
            ...
            <!-- Error -->
            <div v-if="logModal.formError.value" class="px-4 py-3 rounded-xl bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-sm font-semibold flex items-start gap-2 leading-tight">
              ...
              <span>{{ logModal.formError.value }}</span>
            </div>
          ...
          <template #footer>
            <div class="flex items-center gap-3 mt-2 w-full justify-end">
              <n-button size="large" @click="logModal.close()" class="font-bold">Cancel</n-button>
              <n-button type="primary" size="large" @click="logModal.save()" class="font-bold px-6 shadow-md">Save</n-button>
            </div>
          </template>
        </n-card>
      </n-modal>
      ...
      <!-- ── Drive Conflict Resolution Modal ──────────────────────────────── -->
      <n-modal v-model:show="showConflictModal">
        <n-card
          style="width: 340px; border-radius: 28px;"
          title="Google Drive Backup Found"
          :bordered="false"
          size="medium"
          role="dialog"
          aria-modal="true"
        ...
        >
          <div class="flex flex-col gap-3 pt-1">
            <p class="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
              A backup already exists in Google Drive. How would you like to proceed?
            </p>
            <ul class="text-xs text-slate-500 dark:text-slate-400 space-y-1 list-disc list-inside">
              <li><strong>Overwrite local</strong> — replace this device's data with the Drive backup.</li>
              <li><strong>Keep local</strong> — merge Drive data in, local readings take priority.</li>
            </ul>
          </div>
          <template #footer>
            <div class="flex items-center gap-3 mt-2 w-full justify-end">
              <n-button size="large" @click="resolveConflict(false)" class="font-bold">Keep Local</n-button>
              <n-button type="primary" size="large" @click="resolveConflict(true)" class="font-bold px-6 shadow-md">Overwrite Local</n-button>
            </div>
          </template>
        </n-card>
      </n-modal>
